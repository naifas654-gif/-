...
async def bot_logic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for word in BAD_WORDS:
        if word in text.lower():
            ...
            await message.reply_text(f"⚠️ {user.mention_html()}، تم حذف رسالتك! الإنذار ({warn_memory[user.id]}/3)", parse_mode='HTML')
            return
    ...
    if message.reply_to_message or "@" in text:
        ...
        if command == "حذف":
            ...
            if warn_memory[target.id] >= 3:
                ...
                await message.reply_text(f"⚠️ {target.first_name} تم كتمه نهائياً!")
            else:
            ...
            if warn_memory.get(target.id, 0) > 0:
                ...
                await message.reply_text(f"تمت إزالة إنذار. الرصيد: {warn_memory[target.id]}/3 ✅")
            ...
            warn_memory[target.id] = 0
            await message.reply_text(f"تمت إزالة الكتم عن {target.first_name} ✅")

if __name__ == '__main__':
    threading.Thread(target=run_flask).start()
    bot_app = ApplicationBuilder().token(TOKEN).build()
    bot_app.add_handler(MessageHandler(filters.TEXT & (~filters.COMMAND), bot_logic))
    bot_app.run_polling()
